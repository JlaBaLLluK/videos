...
<template>
  ...
  <div v-else>
    ...
    <v-list>
      <v-list-item v-for="(comment, index) in comments" :key="comment.id" class="border-b pa-4">
        <single-comment v-model="comments[index]" @comment-edit="$emit('commentPublished')" @comment-delete="$emit('commentDeleted', comment.id)" />
      </v-list-item>
    </v-list>
  </div>
</template>
